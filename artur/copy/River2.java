...
public class River2 extends MapObject{
	...
	*//**
	...
	while(!done){
		...
		System.out.println(" ref1Y: " + reference1.retY() + "ref2Y:  "+reference2.retY());
		...
		case -1:
			...
			case -1:
				...
				if(flags[0] && flags[1] && flags[2] && flags[3] && flags[4]) {
					System.out.println("Wszystkie flagi ustawione!");
	...
	public int addRiverTile(int dx, int dy, WorldMap2 worldMap){
		...
		if(worldZValue<reference1.retZ() && !river.contains(new Point(reference1.retX()-1,reference1.retY()-1,worldZValue)) && worldMap.getMatrix()[reference1.retX()+dx][reference1.retY()+dy].retId()!=this.retValue()){
			if(worldZValue==0 //|| 
